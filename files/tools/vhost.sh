...
EOF_HELP
    echo "$help" | sed "s/###php_versions###/$(get_php_versions)/g"
    echo ""
}
...
function add_vhost {
    ...
    if [ ! -f "$CertPath/$ServerName.crt" ] && [ ! -f "$KeyPath/$ServerName.key" ]; then
        ...
        -subj "/C=CA/ST=Ontario/L=Toronto/O=Bounteous/OU=Development/CN=$ServerName"
...
function vhost_list {
    ...
    for config in ${configs[@]} ; do
        filename=${config}
        file=$(cat $filename)
        serverAlias=$(echo "$file" | head -n20 | grep 'ServerAlias' | xargs | sed "s/ServerAlias /-a*/g")
