...
if [ ! -f /etc/elasticsearch/elasticsearch.yml ] || apt-cache policy elasticsearch | grep -q "Installed: 7."; then
    cd /tmp

    if apt-cache policy elasticsearch | grep -q "Installed: 7."; then
