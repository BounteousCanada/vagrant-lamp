...
function sync_db {
    ...
    # GET LIST OF TABLES
    tables=`$MYSQL $REMOTECONNECTIONSTR --batch -N -e "select table_name from information_schema.tables where table_name not like '% %' and table_name not like '%-%' and table_type='BASE TABLE' and table_schema='$RemoteDatabase';" 2>&1 | grep -a -v "Warning: Using a password"`
    for table in $tables; do
        if [[ $table == "dataflow_"* ]] || [[ $table == "log_"* ]] || [[ $table == *"_cl" ]] ; then
            ...
            --skip-comments --skip-disable-keys --default-character-set=latin1 --skip-triggers --single-transaction  $RemoteDatabase $table 2>&1 | grep -a -v "Warning: Using a password" | \
            mysql $LOCALCONNECTION  $RemoteDatabase  2>&1 | grep -a -v "Warning: Using a password"
        else
    ...
    #PROCEDURES
    $MYSQLDUMP $REMOTECONNECTIONSTR --compress --quick --routines --no-create-info --no-data --no-create-db --skip-opt --skip-triggers $RemoteDatabase 2>&1 | grep -a -v "Warning: Using a password" | \
    sed -r 's/DEFINER=`[^`]+`@`[^`]+`/DEFINER=CURRENT_USER/g' | mysql $LOCALCONNECTION  $RemoteDatabase 2>&1 | grep -a -v "Warning: Using a password"

    # COPY ALL TRIGGERS
    echo "TRIGGERS "$RemoteDatabase
    #TRIGGERS
    $MYSQLDUMP $REMOTECONNECTIONSTR  --compress --quick --no-create-info --no-data --no-create-db --skip-opt --triggers $RemoteDatabase 2>&1 | grep -a -v "Warning: Using a password" | \
    sed -r 's/DEFINER=`[^`]+`@`[^`]+`/DEFINER=CURRENT_USER/g' | mysql $LOCALCONNECTION  $RemoteDatabase 2>&1 | grep -a -v "Warning: Using a password"

    # COPY ALL VIEWS
    # GET LIST OF ITEMS
    views=`$MYSQL $REMOTECONNECTIONSTR --batch -N -e "select table_name from information_schema.tables where table_name not like '% %' and table_name not like '%-%' and table_type='VIEW' and table_schema='$RemoteDatabase';" 2>&1 | grep -a -v "Warning: Using a password"`
    MYVIEWS=""
    for view in $views; do
        MYVIEWS=${MYVIEWS}" "$view" " 2>&1 | grep -a -v "Warning: Using a password"
    done
    echo "VIEWS "$RemoteDatabase
    if [ -n "$MYVIEWS" ]; then
      #VIEWS
      $MYSQLDUMP $REMOTECONNECTIONSTR --compress --quick -Q -f --no-data --skip-comments --skip-triggers --skip-opt --no-create-db --complete-insert --add-drop-table $RemoteDatabase $MYVIEWS 2>&1 | grep -a -v "Warning: Using a password" | \
      sed -r 's/DEFINER=`[^`]+`@`[^`]+`/DEFINER=CURRENT_USER/g'  | mysql $LOCALCONNECTION  $RemoteDatabase 2>&1 | grep -a -v "Warning: Using a password"
    fi
